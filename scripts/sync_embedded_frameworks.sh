...
# Xcode incremental Debug builds can replace facelivenessdk with a ~50KB Swift stub.
set -euo pipefail
...
copy_framework facelivenessdk
copy_framework FaceLivenessEngine
copy_framework onnxruntime

SDK_BIN="${DEST}/facelivenessdk.framework/facelivenessdk"
if [[ ! -f "${SDK_BIN}" ]]; then
  echo "error: facelivenessdk binary missing after sync" >&2
  exit 1
fi

SIZE=$(/usr/bin/stat -f%z "${SDK_BIN}" 2>/dev/null || /usr/bin/stat -c%s "${SDK_BIN}")
if [[ "${SIZE}" -lt 500000 ]]; then
  echo "error: facelivenessdk is ${SIZE} bytes (expected full native SDK)" >&2
  /usr/bin/otool -L "${SDK_BIN}" 2>&1 | head -5 >&2 || true
  exit 1
fi
if /usr/bin/otool -L "${SDK_BIN}" 2>/dev/null | /usr/bin/grep -q swbuild.tmp; then
  echo "error: facelivenessdk is still an Xcode Swift stub (swbuild.tmp)" >&2
  exit 1
fi

if [[ -n "${EXPANDED_CODE_SIGN_IDENTITY:-}" && "${CODE_SIGNING_ALLOWED:-}" == "YES" ]]; then
  for fw in facelivenessdk FaceLivenessEngine onnxruntime; do
...
echo "sync_embedded_frameworks: facelivenessdk ok (${SIZE} bytes)"
